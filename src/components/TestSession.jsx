            ...
            (async () => {
                try {
                    const res = await axios.post(
                        `http://localhost:8080/Learning-App/Student/generate-practice-test?userId=${user.userId}&subject=${selectedSubject}&topic=${selectedTopic}&difficulty=${selectedDifficulty}&questionCount=${selectedQuestionCount}&timeLimitMinutes=${selectedTimeMinutes}`,
                        {},
            ...
            (async () => {
                try {
                    const res = await axios.get(
                        `http://localhost:8080/Learning-App/Test/get-test?testId=${TeacherTestId}`,
                        { headers: { Authorization: `Bearer ${token}` } }
                    );
